...
export default function TemperatureChart() {
    const [selectedSensor, setSelectedSensor] = useState<Device | undefined>(
        undefined
    );
    // const [temperatureData, setTemperatureData] = useState<
    //     TemperaturesByDevice | undefined
    // >(undefined);
    ...
    useEffect(() => {
        const fetchDevices = async () => {
            ...
            try {
                ...
                toast("Error", {
                    description: "Failed to load sensor devices",
                });
            } finally {
        ...
        const fetchData = async () => {
            ...
            try {
                const sensor =
                    sensorDevices.find(
                        (s) =>
                            s.device_address === selectedSensor.device_address
                    ) || sensorDevices[0];
                const dataApi =
                    await temperatureApiRequest.getTemperatureByDevice(
                ...
                const { dailyData, yearlyData } = generateSensorData(tempData);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const data: any = tempData.temperatures.map((x) => ({
                ...
                const todayTemp =
                    dailyData[dailyData.length - 1]?.temperature || 0;
                const weeklyAvg = dailyData.length
                    ...
                    : 0;
                const monthlyAvg =
                    yearlyData[new Date().getMonth()]?.temperature || 0;

                setStats({ todayTemp, weeklyAvg, monthlyAvg });
            } catch (error) {
                console.error("Failed to fetch temperature data:", error);
                toast("Error", {
                    description: "Failed to load temperature data",
                });
            } finally {
        ...
        <div className="space-y-6 w-full">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <Select
                        value={selectedSensor?.device_id}
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                        <div>
                            <CardTitle className="flex items-center gap-2">
                                {selectedSensor?.device_name ||
                                    "No Sensor Selected"}
                                <Badge
                                    variant={
                                        selectedSensor?.device_battery ??
                                        100 > 30
                                            ? "default"
                                            : "destructive"
                                    }
                                >
                                    {selectedSensor?.device_battery ?? 100 > 30
                                        ? "Active"
                                        : "Low Battery"}
                                </Badge>
                            </CardTitle>
                            <CardDescription className="flex items-center gap-1">
                                ...
                                {selectedSensor?.device_location || "N/A"}
                            </CardDescription>
                        </div>
                        <div className="mt-2 sm:mt-0">
                            ...
                            <Badge variant="outline">
                                Type:{" "}
                                {getDeviceTypeName(
                                    selectedSensor?.device_type ?? 0
                                )}{" "}
                                v{selectedSensor?.device_version || "N/A"}
                            </Badge>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">
                                IP: {selectedSensor?.device_ip || "N/A"}
                            </span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-sm text-muted-foreground flex items-center gap-1">
                            ...
                            <span className="font-medium">
                                {selectedSensor?.device_sampling_rate || 0} min
                            </span>
                            <span className="text-xs text-muted-foreground">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                                    <div
                                        className={`h-full ${getBatteryStatusColor(
                                            selectedSensor?.device_battery ??
                                                100
                                        )}`}
                                        style={{
                                ...
                                <span className="font-medium">
                                    {selectedSensor?.device_battery ?? 0}%
                                </span>
                            </div>
                        </div>
            ...
            {isLoadingData ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <StatCard
                        title={`${
                            selectedSensor?.device_name || "Sensor"
                        } - Current`}
                        value={`${stats.todayTemp}°C`}
                        description="Current reading"
                        statusColor={getStatusColor(
                            stats.todayTemp,
                            selectedSensor?.device_threshold ?? 0
                        )}
