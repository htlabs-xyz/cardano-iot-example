...
export class StatusManagement extends MeshAdapter {
  ...
  lock = async ({
    ...
    title: string;
    authorityPaymentKeyHash?: string;
    isLock: number;
  }) => {
    const { utxos, collateral, walletAddress } = await this.getWalletForTx();
    const ownerPaymentKeyHash = deserializeAddress(
      process.env.WALLET_ADDRESS_OWNER as string,
    ).pubKeyHash;
  ...
  unLock = async ({
    ...
    if (!utxo) {
      ...
      unsignedTx
        ...
        .txOutInlineDatumValue(
          mConStr0([ownerPaymentKeyHash, authorityPaymentKeyHash, isLock]),
        );
    }
