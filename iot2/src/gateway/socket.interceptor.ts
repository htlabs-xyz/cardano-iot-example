...
import { ApiResponseModel, RESPONSE_MESSAGE_METADATA } from '../common/response.interceptor';
import { WsException } from '@nestjs/websockets';
import { EVENT_NAME_KEY } from './socket.decorator';

@Injectable()
export class ResponseWsInterceptor<T> implements NestInterceptor<T, ApiResponseModel<T>> {
    ...
    errorHandler(exception: WsException | HttpException, context: ExecutionContext) {
        ...
        const eventName = this.reflector.get<string>(EVENT_NAME_KEY, context.getHandler()) || 'defaultEvent';
    ...
    responseHandler(res: any, context: ExecutionContext) {
        ...
        const message =
            this.reflector.get<string>(RESPONSE_MESSAGE_METADATA, context.getHandler()) || 'success';  // Lấy thông điệp metadata
