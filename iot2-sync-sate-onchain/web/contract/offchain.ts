...
export class LockerContract extends MeshAdapter {
    ...
    public init = async ({ title }: { title: string }): Promise<string> => {
        ...
        if (utxo) {
            throw new Error("This locker has already been initialized")
        }
    ...
    public lock = async ({ title }: { title: string }): Promise<string> => {
        ...
        if (!utxo) {
            signerHash = deserializeAddress(walletAddress).pubKeyHash;
            unsignedTx
                .mintPlutusScriptV3()
                .mint('1', this.policyId, stringToHex(title))
                .mintingScript(this.mintScriptCbor)
            ...
            signerHash = deserializeAddress(datum.authorized).pubKeyHash;
            unsignedTx
    ...
    public unLock = async ({ title }: { title: string }): Promise<string> => {
        ...
        const signerHash = deserializeAddress(datum.authorized).pubKeyHash;
