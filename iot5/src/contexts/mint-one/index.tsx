...
export default function MintOneProvider({
    ...
    useEffect(() => {
        ...
        setTransportationToMint(null!);
        setCollectionToSave(null!);
        resetTasks();
    }, [resetTasks, setBasicInfoToMint, setCollectionToSave, setMetadataToMint, setTransportationToMint]);
    ...
    const productMetadata = useMemo(() => {
        ...
        const { name, imageUrl, description, Certification, Document, ProductionProcess, WarehouseStorage } =
            productData.data;
        ...
        const flattenedCertificates = Certification?.reduce(
            (acc, cert, index) => {
                ...
                acc[`cerfiticate_issue_date_${index + 1}`] = cert.issueDate
                    ? cert.issueDate.toISOString()
                    : `Certificate Issue Date ${index + 1}`;
                acc[`cerfiticate_expiry_date_${index + 1}`] =
                    cert.expiryDate instanceof Date
                        ? cert.expiryDate.toISOString()
                        : cert.expiryDate || `Certificate Expiry Date ${index + 1}`;
                return acc;
            },
        ...
        const flattenedWarehouse = WarehouseStorage?.reduce(
            (acc, warehouse, index) => {
                ...
                acc[`warehouse_entry_date_${index + 1}`] = warehouse.entryTime
                    ? warehouse.entryTime.toISOString()
                    : `Warehouse Entry Date ${index + 1}`;
                acc[`warehouse_exit_date_${index + 1}`] =
                    warehouse.exitTime instanceof Date
                        ? warehouse.exitTime.toISOString()
                        : warehouse.exitTime || `Warehouse Exit Date ${index + 1}`;
                return acc;
            },
            {} as Record<string, string>,
        );
        ...
        const flattenedProcesses = ProductionProcess?.reduce(
            (acc, process, index) => {
                ...
                acc[`process_start_date_${index + 1}`] = process.startTime
                    ? process.startTime.toISOString()
                    : `Process Start Date ${index + 1}`;
                acc[`process_end_date_${index + 1}`] =
                    process.endTime instanceof Date
                        ? process.endTime.toISOString()
                        : process.endTime || `Process End Date ${index + 1}`;
                return acc;
            },
            {} as Record<string, string>,
        );
